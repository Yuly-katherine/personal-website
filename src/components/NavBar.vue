<template>
  <header class="flex justify-between items-center p-6 bg-opacity-50 relative z-20" >
    <!---NAVBAR LINK--->
    <nav :class="['fixed inset-0 z-20 flex flex-col items-center justify-center bg-[#111827] md:relative md:bg-transparent md:flex md:justify-center', isMenuOpen ? 'block': 'hidden']">
        <ul class="flex flex-col items-center space-y-5 md:flex-row md:space-x-5 md:space-y-0">
            <li v-for="item in Menu" :key="item.name">
                <a :href="item.href" 
                @click="scrollToSection(item.href)"
                class="block text-white transition hover:text-[#f59e0b] ease-linear text-2xl md:text-lg"
                >{{ item.name }}</a>
            </li>
        </ul>
    </nav>
